...
export class Board {
  ...
  public initCells() {
    for (let i = 0; i < 8; i++) {
      ...
      for (let j = 0; j < 8; j++) {
        ...
        //когда строка сформирована во внутреннем массиве
        //будем ее добавлять ее
        //А во внутреннем цикле формируем столбец строк = доску
      }
      this.cells.push(row);
    }
  }
  ...
  //метод чтобы точечно заставлять фигуры на нужные ячейки
  public getCell(x: number, y: number) {
    return this.cells[y][x];
  }

  //методы расстановки фигур на стратовые позиции
  private addPawns() {
    for (let i = 0; i < 8; i++) {
